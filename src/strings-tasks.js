...
function reverseWords(str) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    const newWord = array[i].split('').reverse().join('');
    if (i < array.length - 1) {
      result += `${newWord} `;
    } else {
      result += `${newWord}`;
...
function invertCase(str) {
  ...
  for (let i = 0; i < newStr.length; i += 1) {
    char =
      newStr[i].toString() === newStr[i].toString().toLowerCase()
        ? newStr[i].toString().toUpperCase()
        : newStr[i].toString().toLowerCase();
    result += char;
  }
  return result;
}
...
function extractNameFromTemplate(value) {
  ...
  const result = `${nameData[1]} ${nameData[2]}`;
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const key = alphabet.indexOf(str[i]);
